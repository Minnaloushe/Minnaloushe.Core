...
public class KafkaConsumerSimpleClientProvider(
    string connectionName,
    KafkaClientOptions options,
    ILogger<KafkaConsumerSimpleClientProvider> logger,
    ...
    : IKafkaConsumerClientProvider, IAsyncInitializer
{
    ...
    public IClientLease<IKafkaConsumerClientWrapper> Acquire()
        => _holder.Acquire();
    ...
    private async Task<IKafkaConsumerClientWrapper> CreateClient(CancellationToken cancellationToken)
    {
        ...
        {
            endpoint = (await serviceDiscovery.ResolveServiceEndpoint(_options.ServiceName, cancellationToken))
                ?.FirstOrDefault();
        }

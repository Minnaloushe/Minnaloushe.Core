...
public class KafkaVaultIntegrationTests
{
                ...
                MessageQueues = new
                {
                    Connections = new[]
                    {
                        MqHelpers.CreateConnection(_connectionName,
                            ...
                            serviceKey: _serviceKey, serviceName: GlobalFixture.Kafka1.Name, username: string.Empty, password: string.Empty)
                    },
                    Consumers = new[]
                    {
    ...
    public async Task OneTimeSetup()
    {
        ...
        _mockServiceDiscovery.Setup(x => x.ResolveServiceEndpoint(
                GlobalFixture.Kafka1.Name,
                It.IsAny<CancellationToken>()))
            .ReturnsAsync([new ServiceEndpoint
            {
                Host = GlobalFixture.Kafka1.Host,
                Port = GlobalFixture.Kafka1.Port
            }]);
    ...
    {
        var clientProvider = _testHost.Services.GetKeyedService<IKafkaConsumerClientProvider>(_connectionName);
...
#pragma warning restore CA1873
            KafkaVaultIntegrationTests.ReceivedMessages.Add(envelop.Message);
